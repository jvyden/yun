...
// still big spaghetti
const rules = ["1. Do not talk about /b/",
...
export default class extends Command {
  constructor() {
    super("rule", {
      aliases: ["rule"],
      description: "Gives you a random rule of the internet. Give a number to see a specific rule.",
      editable: true,
      args: [{
        id: "rule"
      }]
  ...
  public async exec(message: Message, { rule }: any) {
    if(!rule) {rule = Math.floor(Math.random() * rules.length);}
    rule -= 1
    if(!rules[rule]) {
      return message.channel.send(`That rule doesn't exist. You can see a rule of the internet by picking a number between 1 and ${rules.length}.`)
    }
    return message.channel.send(rules[rule])
  }
}
