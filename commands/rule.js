...
const info = {
  helptext: "Gives you a random rule of the internet. Give a number to see a specific rule."
}
function execute(message, args) {
  let r = args[0]
  if(!r) {r = Math.floor(Math.random() * rules.length);}
  r -= 1
  if(!rules[r]) {
    return `That rule doesn't exist. You can see a rule of the internet by picking a number between 1 and ${rules.length}.`
  }
  return rules[r]
}
module.exports = {
